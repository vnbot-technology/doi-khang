...
func _do_special() -> void:
	...
	velocity.x += (1.0 if facing_right else -1.0) * 440.0
			attack_hitbox.damage = 22.0; attack_hitbox.knockback_force = 320.0
...
func _do_ultimate() -> void:
	...
	if attack_hitbox:
		...
		attack_hitbox.scale.x = 1.0 if facing_right else -1.0
		attack_hitbox.monitoring = true
	velocity.x += (1.0 if facing_right else -1.0) * 260.0
			attack_hitbox.damage = 50.0; attack_hitbox.knockback_force = 480.0
